...
class DatabaseStore: ObservableObject {
    ...
    func storePost(post: Post,completion: @escaping (Bool) -> ()) {
        ...
        if let uid = post.uid{
            ...
            store.collection(USER_PATH).document(uid).collection(POST_PATH).document(postId).setData(params as [String : Any])
            store.collection(USER_PATH).document(uid).collection(FEED_PATH).document(postId).setData(params as [String : Any])
            completion(true)
    ...
    func followUser(me: User, to: User,completion: @escaping (Bool) -> ()) {
        ...
        store.collection(USER_PATH).document(me.uid!).collection(FOLLOWING_PATH).document(to.uid!).setData(paramsTo as [String : Any])
        
        // I am in someone`s followers
        store.collection(USER_PATH).document(to.uid!).collection(FOLLOWERS_PATH).document(me.uid!).setData(paramsMe as [String : Any])
    ...
    func unFollowUser(me: User, to: User,completion: @escaping (Bool) -> ()) {
        ...
        _ = ["uid": me.uid, "displayName": me.displayName, "imgUser": me.imgUser]
        
        _ = ["uid": to.uid, "displayName": to.displayName, "imgUser": to.imgUser]
        
        store.collection(USER_PATH).document(me.uid!).collection(FOLLOWING_PATH).document(to.uid!).delete()
        
        // I am not in someone`s followers
        store.collection(USER_PATH).document(to.uid!).collection(FOLLOWERS_PATH).document(me.uid!).delete()
    ...
    func removeFeed(uid: String, post: Post) {
        ...
        if let postId = post.postId{
            store.collection(USER_PATH).document(uid).collection(FEED_PATH).document(postId).delete()
    ...
    func likeFeedPost(uid: String, post: Post) {
        ...
        if let postId = post.postId{
            store.collection(USER_PATH).document(uid).collection(FEED_PATH).document(postId).updateData(["isLiked": post.isLiked!])
            if post.uid == uid {
    ...
    func removeMyPost(uid: String, post: Post) {
        ...
        if let postId = post.postId{
            store.collection(USER_PATH).document(uid).collection(FEED_PATH).document(postId).delete()
            store.collection(USER_PATH).document(uid).collection(POST_PATH).document(postId).delete()
